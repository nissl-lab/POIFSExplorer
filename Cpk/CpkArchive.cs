    ...
    public class CpkArchive
    {
        ...
        public async Task<byte[]> ReadAsync(string fileVirtualPath)
        {
            ...
            if (_archiveInMemory)
            {
                ...
                var streamData = new byte[end-start];
                stream = new MemoryStream(new ArraySegment<byte>(_archiveData, start, end).ToArray());
            }
            else
            {
            ...
            try
            {
                ...
                if (table.IsCompressed())
                {
                    ...
                    var lzoStream = new LzoStream(stream, CompressionMode.Decompress);
        ...
        private void LoadArchiveIntoMemory()
        {
        ...
        private async Task BuildFileNameMap()
        {
            ...
            try
            {
                foreach (var table in _tables)
                {
                    ...
                    await stream.ReadAsync(extraInfo, 0, extraInfo.Length);
